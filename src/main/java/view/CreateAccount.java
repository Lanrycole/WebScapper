...
public class CreateAccount extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            out.println("<div style=\"display: inline-block; text-align: left;\"> Create Account <br>");
            out.println("<form method=\"post\">");

            //instead of typing the name of column manualy use the static vraiable in logic
            //use the same name as column id of the table. will use this name to get date
            //from parameter map.
            out.printf("NAME:     <input type=\"text\" name=\"%s\" value=\"\"><br>", AccountLogic.DISPLAY_NAME);
            out.printf("USER:     <input type=\"text\" name=\"%s\" value=\"\"><br>", AccountLogic.USER);
            out.printf("PASS: <input type=\"text\" name=\"%s\" value=\"\"><br>", AccountLogic.PASSWORD);

            out.println("<br>");
            out.println("<input type=\"submit\" name=\"view\" value=\"Add Account and View\">");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        String name = request.getParameter(AccountLogic.DISPLAY_NAME);
        //since it is unique we check for duplicates.

        try {

            if (accountlogic.getAccountWithDisplayName(name) == null) {
                //if no duplicates create the entity and add it.
                Account accountget = accountlogic.createEntity(request.getParameterMap());
                accountlogic.add(accountget);
            } else {
